 ...
 * @author Thomas
 */
public abstract class StringInputButton extends CanvasButton {

    float timePassed = 0;
    char caret = '|';
    private String previousText = getText();

    public StringInputButton(String label, Color color, int x, int y) {
        super(label, color, x, y);
    }

    public StringInputButton(String label, Color color, Font font, int x, int y) {
        super(label, color, font, x, y);
    }

    public StringInputButton(String label, Color color, int x, int y, int width, int height) {
        super(label, color, x, y, width, height);
    }

    public StringInputButton(String label, Color color, Font font, int x, int y, int width, int height) {
    ...
    public void draw(Graphics2D g2d, float delta) {
        ...
        if (isFocussed() && (int) timePassed % 2 == 0) {
            ...
            if (!isFocussed() && previousText != null && timedMessage == null && getText().isEmpty()) {
                ...
                previousText = null;
            } else if (!isFocussed() && previousText != null && !previousText.equals(getText()) && !getText().isEmpty()) {
                onSetString(getText());
                previousText = getText();
                text = getText();
    ...
    public abstract void onSetString(String string);
